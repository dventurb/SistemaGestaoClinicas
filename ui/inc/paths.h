#ifndef PATHS_H
#define PATHS_H 

#define STYLE_CSS_PATH "styles/styles.css"

#define LOGOTIPO_PATH "icon/icon.png"

#define DASHBOARD_ICON_PATH "icon/dashboard.png"
#define CLIENT_ICON_PATH "icon/client.png"
#define DOCTOR_ICON_PATH "icon/doctor.png"
#define APPOINTMENT_ICON_PATH "icon/appointment.png"
#define SETTINGS_ICON_PATH "icon/settings.png"

#define BACK_ICON_PATH "icon/back.png"

#define ADD_CLIENT_PATH "icon/add-client.png"
#define EDIT_CLIENT_PATH "icon/edit-client.png"
#define TOGGLE_CLIENT_PATH "icon/toggle-client.png"
#define VIEW_CLIENT_PATH "icon/view-client.png"

#define SUBMIT_PATH "icon/submit.png"

#endif
